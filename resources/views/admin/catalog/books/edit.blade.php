<x-admin.layout title="Editar Livro - Painel Administrativo">
<div class="max-w-3xl mx-auto">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8 pb-6 border-b border-slate-200/80">
            <div>
                ...
                <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Editar Livro</h1>
        ...
        <div class="bg-white/80 backdrop-blur-sm rounded-3xl shadow-sm border border-slate-200/80 p-6 sm:p-8">
            <form action="{{ route('admin.books.update', $book->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    <label for="admin-title" class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Título do Livro</label>
                    <div class="relative">
                        <span class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-slate-400">
                            <i class="fa-solid fa-book text-xs"></i>
                        </span>
                        <input id="admin-title" type="text"
                               name="title"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div>
                        <label for="admin-page_count" class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Número de Páginas</label>
                        <div class="relative">
                            <span class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-slate-400">
                                <i class="fa-solid fa-file-lines text-xs"></i>
                            </span>
                            <input id="admin-page_count" type="number"
                                   name="page_count"
                    ...
                    <div>
                        <label for="admin-fk_author_id" class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Autor</label>
                        <div class="relative">
                            <select id="admin-fk_author_id" name="fk_author_id"
                                    required
                                    class="w-full pl-4 pr-10 py-3 bg-slate-50/50 border border-slate-200 rounded-2xl text-slate-800 text-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all appearance-none">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div>
                        <label for="admin-fk_genre_id" class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Gênero</label>
                        <div class="relative">
                            <select id="admin-fk_genre_id" name="fk_genre_id"
                                    required
                                    class="w-full pl-4 pr-10 py-3 bg-slate-50/50 border border-slate-200 rounded-2xl text-slate-800 text-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all appearance-none">
                    ...
                    <div>
                        <label for="admin-fk_availability_id" class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Disponibilidade</label>
                        <div class="relative">
                            <select id="admin-fk_availability_id" name="fk_availability_id"
                                    required
                                    class="w-full pl-4 pr-10 py-3 bg-slate-50/50 border border-slate-200 rounded-2xl text-slate-800 text-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all appearance-none">
                ...
                <div>
                    <label for="admin-description" class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Descrição / Sinopse</label>
                    <textarea id="admin-description" name="description"
                              rows="4"
                              class="w-full p-4 bg-slate-50/50 border border-slate-200 rounded-2xl text-slate-800 text-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all resize-none">{{ old('description', $book->description) }}</textarea>
                </div>
                ...
                <div class="pt-4 border-t border-slate-100">
                    ...
                    <div class="flex flex-col sm:flex-row items-start sm:items-center gap-6 p-4 bg-slate-50/80 rounded-2xl border border-slate-200/60">
                        ...
                        <div class="flex-1 w-full">
                            ...
                            <input type="file"
                                   name="cover_image"
                                   class="w-full text-xs text-slate-500 file:mr-4 file:py-2.5 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 transition-all cursor-pointer">
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-end gap-3 pt-6 border-t border-slate-100">
                    ...
                    <button type="submit"
                            class="px-6 py-2.5 rounded-xl font-medium text-sm shadow-md shadow-blue-600/20 hover:shadow-blue-600/30 transition-all bg-blue-600 text-white hover:bg-blue-700">
                        Atualizar Livro
                    </button>
                </div>
            </form>
